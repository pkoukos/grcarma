...
sub carma {
    ...
    # monitor the program in real time     #
        ...
        if ( $_[0] && $_[0] eq 'sort' ) {
            ...
            `xterm -geometry 80x50+800+200 -e "carma $flag $remember_dcd | tee carma.out.copy"`;
        }
        elsif ( $have_custom_psf ) {

            if ( $dcd_count >= 0 && $_[0] ne "fit" ) {

                `xterm -geometry 80x50+800+200 -e "carma $flag selected_residues.psf carma_fitted_$dcd_count.dcd | tee carma.out.copy"`;
                $remember_psf = "selected_residues.psf" if ( $_[0] && $_[0] eq "pca" );
                $remember_dcd = "carma_fitted_$dcd_count.dcd" if ( $_[0] && $_[0] eq "pca" );
            }
            else {

                `xterm -geometry 80x50+800+200 -e "carma $flag selected_residues.psf $dcd_file | tee carma.out.copy"`;
                $remember_psf = "selected_residues.psf" if ( $_[0] && $_[0] eq "pca" );
            ...
            if ( $dcd_count >= 0 && $_[0] ne "fit" ) {
                ...
                `xterm -geometry 80x50+800+200 -e "carma $flag carma_fitted_$dcd_count.psf carma_fitted_$dcd_count.dcd | tee carma.out.copy"`;
                $remember_psf = "carma_fitted_$dcd_count.psf" if ( $_[0] && $_[0] eq "pca" );
                $remember_dcd = "carma_fitted_$dcd_count.dcd" if ( $_[0] && $_[0] eq "pca" );
            }
            else {

                `xterm -geometry 80x50+200+200 -e "carma $flag $psf_file $dcd_file | tee carma.out.copy"`;
    ...
    open TEMP_OUT, "carma.out.copy" || die "Cannot open carma.out.copy for writing\n";
    while ( <TEMP_OUT> ) {
...
sub dpca_window {
    ...
    if ( !Exists( my $dpca_top ) ) {
        ...
        my $dpca_frame_3 = $dpca_top -> Frame()-> pack( -side => 'top',
                                                        -expand => 1,
                                                        -fill => 'x', );
...
sub cpca_window {
    ...
    if ( !Exists( my $cpca_top ) ) {
        ...
        $cpca_frame ->Button( -text => 'Run',
                              -command => sub {
                                ...
                                if ( $seg_id_flag ) {
                                    ...
                                    $flag = " -v -w -col -cov $res_id_flag $cpca_dgwidth $cpca_dgwidth_num $seg_id_flag -eigen -proj $cpca_eigenvectors $cpca_combinations $cpca_temp $cpca_cutoff $cpca_mass $cpca_3d $cpca_use";
                                }
                                else {

                                    $flag = " -v -w -col -cov $res_id_flag $cpca_dgwidth $cpca_dgwidth_num -eigen -proj $cpca_eigenvectors $cpca_combinations $cpca_temp $cpca_cutoff $cpca_mass $cpca_3d $cpca_use";
                                }
                                &create_dir;
                                &carma ( "pca" );
        ...
        $cpca_frame_4 -> Label( -text => 'Various Options' )
                                -> pack( -side => 'top', );
        ...
        $cpca_frame_4 -> Checkbutton( -text => 'Create 3D landscapes',
                                      ...
                                      -onvalue => " -3d", )
                                      -> pack( -side => 'left', -anchor => 'w', );
        $cpca_frame_4 -> Checkbutton( -text => 'Use',
                                      -variable => \$cpca_use,
                                      -offvalue => '',
                                      -onvalue => " -use", )
                                      -> pack( -side => 'left', -anchor => 'w', );
...
sub create_fit_index {
    ...
    open PSF_FILE, "carma.selected_atoms.psf" || die "Cannot open carma.selected_atoms.psf for reading\n";
