...
# Do not allow the use of undeclared   #
# variables and warn when an           #
...
unless ( $run_from_terminal ) {
	...
	my $y_position = int ( ( $mw -> screenheight / 2 ) - ( $mw -> height / 2 ) );
...
$f0 -> waitVariable(\$have_files) unless ( $have_files );
...
if ( $^O eq 'linux' ) {
	...
	if ( $ps_viewer eq 'evince' ) {

		$text -> insert( 'end', "\nThe program selected for .ps viewing is \"$ps_viewer\"\n", 'info' );
	}
	elsif ( $ps_viewer eq 'gs' ) {

		$text -> insert( 'end', "\nThe program selected for .ps viewing is \"$ps_viewer\"\n", 'info' );
	}
	elsif ( $ps_viewer eq 'display' ) {
...
sub carma {
	...
	if ( $^O ne 'linux' ) {
		...
		$text -> insert( 'end', "\nStarting the calculation with the flag :\n", 'valid' );
		...
		if ( $_[0] && $_[0] eq 'auto' ) {
			...
			`carma.exe $flag | tee carma.out.copy`;
		}
...
sub dpca_window {
	...
	my $chi1 = '';
	my $dpca_auto_entry;
	my $dpca_auto_entry_num;

	our $res_id_flag;

	if ( !Exists( my $dpca_top ) ) {
		...
		$dpca_auto_entry = $dpca_frame_3 -> Entry( -textvariable => $dpca_auto_entry_num,
												   -state => 'disabled', )
												   -> grid( -row => 2, -column => 2, );

		$dpca_frame_3 -> Checkbutton( -text => 'Automatically perform ......................',
									  -command => sub {
									  ...
									  -> grid( -row => 2, -column => 1, -sticky => 'w', );
		$dpca_frame_3 -> Checkbutton( -text => 'Create 3D landscapes',
									  -variable => \$dpca_3d,
									  -offvalue => '',
									  -onvalue => " -3d", )
									  -> grid( -row => 3, -column => 1, -sticky => 'w', );
		$dpca_frame_3 -> Checkbutton( -text => 'Chi1',
									  -variable => \$chi1,
									  -offvalue => '',
									  -onvalue => " -chi1", )
									  -> grid( -row => 4, -column => 1, -sticky => 'w', );
...
sub cpca_window {
	...
	if ( !Exists( my $cpca_top ) ) {
		...
		$cpca_frame_4 -> Checkbutton( -text => 'Automatically perform ......................',
									  -command => sub {
	...
	for ( $i = 1 ; $i <= $clusters ; $i++ ) {
		...
		if ( $seg_id_flag ) {
			...
			if ( $custom_id_flag ) {
				...
				{
					local $" = '|';
					$regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @selected_atoms ] })(.*)};
				}
				...
				my @selected_atoms = split ' -atmid ', $atm_id_flag if ( $atm_id_flag !~ /(HEAVY|ALLID)/ );
				...
				{
					local $" = '|';
					$regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @selected_atoms ] })(.*)} if ( @chains && @selected_atoms );
					$regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)([^H]*)(.*)} if ( $atm_id_flag =~ /HEAVY/ );
					$regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)(\w*)(.*)} if ( $atm_id_flag =~ /ALLID/ );
				}
				...
				{
					local $" = '|';
					$regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @backbone ] })(.*)};
				}
			...
			if ( $custom_id_flag ) {
				...
				{
					local $" = '|';
					$regex_var = qr{^(\s*)\d*(\s*)(Z)(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @selected_atoms ] })(.*)};
				}
				...
				{
					local $" = '|';
					$regex_var = qr{^(\s*)\d*(\s*)(Z)(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @selected_atoms ] })(.*)} if ( @selected_atoms );
					$regex_var = qr{^(\s*)\d*(\s*)(Z)(\s*\d*\s*\w*\s*)([^H]*)(.*)} if ( $atm_id_flag =~ /HEAVY/ );
					$regex_var = qr{^(\s*)\d*(\s*)(Z)(\s*\d*\s*\w*\s*)(\w*)(.*)} if ( $atm_id_flag =~ /ALLID/ );
				}
				...
				{
					local $" = '|';
					$regex_var = qr{^(\s*)\d*(\s*)(Z)(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @backbone ] })(.*)};
				}
			...
			{
				local $" = '|';
				$regex_var = qr{^(\s*)\d+(\s*)(\w+)(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @selected_atoms ] })(.*)} if ( @selected_atoms );
			...
			{
				local $" = '|';
				$regex_var = qr{^(\s*)\d*(\s*)(\w*)(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @backbone ] })(.*)};
			}
		...
		if ( $^O eq 'linux' ) {
			...
			`carma -v -w -col -cov -dot -norm -super carma.fitted.cluster_0$i.dcd $psf_file` unless ( $res_id_flag );
			`carma -v -w -col -cov -dot -norm -super carma.fitted.cluster_0$i.dcd \"selected_residues.psf\"` if ( $res_id_flag );
...
sub image_window {
	...
	my $j = 0;
	my @contents = '';
	my $toplevel_position = "240x290" . $toplevel_position;
	...
	$lb -> bind( '<Button-1>', sub {
									...
									`$ps_viewer $selection &` if ( $^O eq 'linux' );
									`start $ps_viewer $selection` if ( $^O ne 'linux' );
								} );
}
...
sub entropy_window {
	...
	if ( !Exists ( my $top_ent ) ) {
		...
		$frame_ent5 -> Button( -text => 'Run',
				...
				# first run the first tenth of the     #
