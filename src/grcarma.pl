...
sub dpca_window {
    ...
    if ( !Exists( $dpca_top ) ) {
        ...
        $dpca_frame_4 -> Checkbutton( -text => 'Automatically perform cluster analysis',
                                      -command => sub {
...
sub cpca_window {
    ...
    if ( !Exists( $cpca_top ) ) {
        ...
        $cpca_frame_4 -> Checkbutton( -text => 'Automatically perform cluster analysis',
                                      -command => sub {
...
sub auto_window {
    ...
    for ( $i = 1 ; $i <= $clusters ; $i++ ) {
        ...
        my @results = ( $seg_custom, $seg_atm, $seg, $res_custom, $res_atm, $res, $custom, $atm, $nothing, );
        ...
        if ( $seg_id_flag ) {
            ...
            if ( $custom_id_flag ) {print '1';
                ...
                {
                    local $" = '|';
                    $regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)(@{ [ map quotemeta, @selected_atoms ] })(.*)} if ( @selected_atoms );
                    $regex_var = qr{^(\s*)\d*(\s*)(@{ [ map quotemeta, @chains ] })(\s*\d*\s*\w*\s*)(\w*)(.*)} if ( $atm_id_flag =~ /(HEAVY|ALLID)/ );
                }
        ...
        $text -> insert( 'end', "\nNow performing superposition of fitted DCD files. ", 'valid', );
        $text -> see( 'end', );
        $mw -> update;
        ...
        $flag = " -v -w -col -cov -dot -norm -super carma.fitted.cluster_0$i.dcd $psf_file" unless ( $res_id_flag );
        ...
        if ( $all_done ) {
            ...
            if ( $seg_custom || $seg_atm ) {
                ...
                $text -> insert( 'end', "\nThe fitting was performed only for the selected atom types of the selected chains", 'valid', );
                $seg_custom = 0;
                $seg_atm = 0;
            }
            elsif ( $seg ) {

                $text -> insert( 'end', "\nThe fitting was performed only for the backbone atoms of the selected chains", 'valid', );
                $seg = 0;
            }
            elsif ( $res_custom || $res_atm ) {

                $text -> insert( 'end', "\nThe fitting was performed only for the selected atom types of the selected residues", 'valid', );
                $res_custom = 0;
                $res_atm = 0;
            }
            elsif ( $res ) {

                $text -> insert( 'end', "\nThe fitting was performed only for the backbone atoms of the selected residues", 'valid', );
                $res = 0;
            }
            elsif ( $custom || $atm ) {

                $text -> insert( 'end', "\nThe fitting was performed only for the selected atom types", 'valid', );
                $custom = 0;
                $atm = 0;
            }
            elsif ( $nothing ) {

                $text -> insert( 'end', "\nThe fitting was performed only for backbone atoms", 'valid', );
                $nothing = 0;
            }
        }
    }

    $text -> insert( 'end', "\nPerformed superposition of  $clusters dcd files.", 'info', ) if ( $all_done );

    my $response = $mw -> messageBox( -message => "Would you like to open a window to the directory of the created PDB files?",
                                      -type => 'yesno',
                                      -icon => 'question', );
    if ( $response eq 'Yes' ) {
...
my $top = $mw -> Toplevel;
        my $file = $top -> getOpenFile( -filetypes => $filetypes, -initialdir => getcwd, );

        if ( $^O eq 'linux' ) {

            system ( "$pdb_viewer $file &" ) if ( $pdb_viewer );
        }
        else {

            `start $file` if ( $pdb_viewer );
        }
    }
}
